...
import {searchRoute} from './tools.js';
...
export async function inspectDevice({nav=globalThis.navigator,scope=globalThis,secure=globalThis.isSecureContext,
  ...
  const [gpu,micAccess,locationAccess,dictation]=await Promise.all([
    runtime.desktopMode?Promise.resolve({supported:false,notRequired:true,reason:'Installed Mac inference uses the CPU.'}):bounded(()=>checkWebGPU(nav,secure),{signal,timeoutMs,fallback:{supported:false,unknown:true,reason:'The graphics check did not finish. Try Refresh device check.'}}),
  ...
  if(runtime.desktopMode){
    const local=runtime.localAI,confirmed=local?.enabled===true&&local.local_only===true;
    row('local-ai','Local AI · Mac CPU',runtime.modelReady?'ready':confirmed&&local.available?'available':'unavailable',runtime.modelReady?`Installed model ${runtime.modelId} runs on this Mac’s CPU. No internet or WebGPU is needed for inference.`:confirmed&&local.available?'The paired Mac helper can load an installed Ollama model on the CPU. Larger models take longer; model downloads are not started here.':local?.message||'The installed Mac helper has not confirmed CPU inference. Reopen MAX-G or try Load local AI.','general');
  }else row('local-ai','Local AI',runtime.modelReady?'ready':gpu.supported?'available':gpu.unknown?'unknown':'unavailable',runtime.modelReady?`Loaded on this device: ${runtime.modelId}.`:gpuDetail+(!gpu.supported?' Local AI chat needs a supported GPU; calculations and file tools still work.':''),'general');
  ...
  row('natural-voice','Natural speaking voice',runtime.desktopMode&&audio?'setup':audio&&worker&&wasm?(runtime.voiceReady?'ready':'setup'):'unavailable',runtime.desktopMode?'The installed Mac companion provides local neural speech. Use Hear MAX-G or Sound help to verify playback. Voice Studio keeps your selected male voice and delivery controls.':audio&&worker&&wasm?'English neural speech runs on this device’s CPU. First use downloads its voice model; use Sound help to test playback.':'Local neural speech needs Web Audio, WebAssembly and browser workers. Check installed voices as an alternative.','voice');
  ...
  const helper=runtime.companion||{},searchConfigured=Boolean(attempt(()=>searchRoute(settings.proxyURL,{connection:settings.searchConnection}).base,'')||helper.paired);
  row('web-search','Live web information',permissions.internet==='deny'?'blocked':hints.online===false?'offline':searchConfigured?'setup':'setup',searchConfigured?'Your selected search route is configured; use Test web search to verify it. The included Cloudflare service uses this device’s internet connection. Weather and places use their own online services.':'The selected companion route needs pairing on this device. Select MAX-G Cloudflare in Connection to use the included public search service. Weather and places use separate online tools.','connection');
  row('offline','Saved data & offline app',runtime.storageOK===false?'blocked':runtime.desktopMode||nav.serviceWorker?.controller?'available':'setup',runtime.storageOK===false?'This browser could not save recent changes. Keep this window open and export your notes.':runtime.desktopMode?'This app’s pages, installed AI models and installed voices are served from this Mac. Web research needs internet. Export important personal notes separately.':nav.serviceWorker?.controller?'The offline app shell is active. AI and voice need separate cached downloads. Browser storage may be cleared or evicted.':'Offline setup is not confirmed. Visit online and reopen after setup. Private data and downloads stay in this browser.','memory');
