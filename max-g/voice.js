  ...
  async speak(text,{engine='system',neuralVoice='am_fenrir',cloneId='',pitch=0,depth=0,expression=.35,language='en-US',profile='Warm',voiceURI='',rate=1,emotion='neutral',signal}={}){
    ...
    if(engine!=='system'){
      ...
      if(engine==='neural'){
        ...
        try{if(!this.neural.ready)await this.permission('internet');if(generation!==this.generation||signal?.aborted)return;await unlocked;if(generation!==this.generation||signal?.aborted)return;return await this.neural.speak(clean,{neuralVoice,pitch,depth,expression,rate,emotion,signal});}
      ...
      try{await this.neural.unlock({signal});for(const chunk of sentenceChunks(clean,260,{firstLimit:120})){if(signal?.aborted||generation!==this.generation)throw new DOMException('Voice stopped.','AbortError');this.onState('thinking');const controls=deliveryControls({rate,pitch,depth,emotion,expression,text:chunk});const audio=await this.helper('synthesize',{text:chunk,mode:'clone',voice:neuralVoice,language:NEURAL_VOICES.find(v=>v.id===neuralVoice)?.language||language,sample_id:cloneId,rate:controls.speed,pitch:controls.pitch,depth:controls.depth/6},{signal});if(signal?.aborted||generation!==this.generation)throw new DOMException('Voice stopped.','AbortError');if(audio.mime!=='audio/wav'||typeof audio.base64!=='string'||audio.base64.length>12000000)throw new Error('The companion returned invalid voice audio.');const bytes=Uint8Array.from(atob(audio.base64),c=>c.charCodeAt(0)),buffer=await this.neural.context.decodeAudioData(bytes.buffer);if(signal?.aborted||generation!==this.generation)throw new DOMException('Voice stopped.','AbortError');await this.neural.play(buffer.getChannelData(0),buffer.sampleRate,{pitch:0,depth:0},{signal});}}finally{signal?.removeEventListener('abort',abort);if(generation===this.generation){this.cloning=false;this.onState('idle');this.neural.scheduleIdle();}}return;
    }
    ...
    try{for(const chunk of sentenceChunks(clean,220,{firstLimit:120})){
      if(signal?.aborted||generation!==this.generation)return;
      let interrupted=false;const controls=deliveryControls({rate,pitch,depth,expression,emotion,text:chunk}),utterance=new SpeechSynthesisUtterance(chunk);
