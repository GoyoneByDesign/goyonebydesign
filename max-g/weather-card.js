...
export function renderWeatherCard(raw,{motion=true,weatherEffects=true}={}){
  ...
  if(data.mode==='week'){
    ...
    for(const row of data.daily){const item=el('li','weather-day'),label=el('time','weather-day-name');label.append(el('span','',dayName(row.date)),el('small','weather-day-date',row.date.slice(5).replace('-','/')));label.dateTime=row.date;label.title=row.date;const weather=weatherCondition({code:row.code},data.units);const art=weatherArt(weather.kind==='clear'?'sun':weather.kind);art.classList.add('weather-art-small');item.append(label,art,el('span','weather-day-condition',weather.label),el('strong','weather-day-range',`${temp(row.high,data.temperatureUnit)} / ${temp(row.low,data.temperatureUnit)}`),el('span','weather-day-rain',row.precipitationProbability===null?'—':number(row.precipitationProbability)+'%'));week.append(item);}card.append(week);
