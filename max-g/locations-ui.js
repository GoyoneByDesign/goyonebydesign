...
export function createLocationHub({getSettings=()=>LOCATION_DEFAULTS,saveSettings=async()=>{},permission=async()=>{},runTask=async fn=>fn(new AbortController().signal),onCancel=()=>{},onNavigate=()=>{},onReply=async()=>{},onClarify=()=>{},toast=()=>{},services={resolvePlace,nearbyPlaces,getDeviceLocation}}={}){
  ...
  function render(){
    ...
    const card=el('section','','card locations-card');card.append(el('h3','Where would you like to go?'),el('p','Search cities and postal codes worldwide. Include the country, or share your current location once.','muted'));
  ...
  async function resolve(query,country,signal,check){
    ...
    return ask('place',`I could not find “${query}”${country?` in ${country}`:''} in the available sources. Coverage varies. Please try a city and country, a district or a street address.`,{query,country});
  }
