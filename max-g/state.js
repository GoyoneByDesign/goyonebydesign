...
export function validateState(raw){
  ...
  clean.chats=(Array.isArray(raw.chats)?raw.chats:[]).slice(0,12).filter(c=>c&&Array.isArray(c.messages)).map(c=>({id:String(c.id||crypto.randomUUID()),title:String(c.title||'Conversation').slice(0,80),messages:c.messages.filter(m=>m&&['user','assistant'].includes(m.role)&&typeof m.content==='string').slice(-40).map(m=>({id:typeof m.id==='string'?m.id.slice(0,80):crypto.randomUUID(),model:typeof m.model==='string'?m.model.slice(0,160):'Unrecorded older reply',role:m.role,content:m.content.slice(0,16000),...(m.role==='assistant'&&validateWeatherCard(m.weatherCard)?{weatherCard:validateWeatherCard(m.weatherCard),weatherNotices:(Array.isArray(m.weatherNotices)?m.weatherNotices:[]).filter(x=>typeof x==='string').slice(0,2).map(x=>x.slice(0,350))}:{}),sources:Array.isArray(m.sources)?m.sources.slice(0,4).filter(x=>x&&typeof x.url==='string').map(x=>({title:String(x.title||'Source').slice(0,200),url:x.url.slice(0,2000)})):[]}))}));
