...
export function renderDisplaySettings({display,onSave,signal}={}){
  ...
  const candidate=()=>normalizeDisplay({readabilityRevision:DISPLAY_DEFAULTS.readabilityRevision,...Object.fromEntries(Object.entries(fields).map(([key,input])=>[key,['uiScale','textSize'].includes(key)?Number(input.value):input.value]))});
