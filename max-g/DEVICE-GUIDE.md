...
**Settings → Connection** explains MAX-G's independent local AI and provides an
optional Google Search link. General replies are generated by the downloaded
WebLLM model on this device; the search service retrieves public source material.
**Ask Gemini** is a separate optional cloud action: review one question, then send
it through MAX-G’s Cloudflare gateway. It works without WebGPU after server setup
and entering your MAX-G support access token for that browser session. Closing
the page clears the token. It does not send memories, attachments or chat history.
[Free-tier setup and privacy](GEMINI-SUPPORT.md)
No subscription to another assistant is needed. Optional hourly learning uses
public web-platform and programming documentation, with notes waiting for your
review in Memory. Existing personal notes and conversations remain intact.
