    ...
    if(social){answer=social.text;answerModel='Local greeting';setOrb('idle',social.emotion);}
      ...
      if(plan){
        ...
        if(state.settings.permissions.internet==='deny'&&localFallback){searchFailed=true;}
          ...
          catch(error){checkRun(run);if(error.name==='AbortError'||!localFallback)throw error;searchFailed=true;$('modelStatus').textContent='Web search unavailable · answering with local knowledge';}}
      }
      if(researchData){
        ...
        if(engine.readyFor(state.settings.model)){
          try{rendering.content.hidden=false;const summary=await generate(research?.[1]||query,sources,files,run,rendering.content);if(summary&&summary!=='[SEARCH]'){answer=summary;answerModel=engine.modelId||state.settings.model;}}
...
async function boot(){shellReady=prepareOfflineShell();if(!await claimOwnership()){$('compatibilityNotice').hidden=false;$('compatibilityNotice').textContent='MAX-G is already open in another window. Close that window, then reload this one to protect your local data.';return;}try{state=await loadState();}catch(error){storageOK=false;toast('Private mode or storage settings prevented saving. This session still works.');}if(desktopMode)state.settings=desktopDefaults(state.settings);else if(!MODELS.some(m=>m.id===state.settings.model))state.settings.model=DEFAULT_MODEL;if(state.settings.saveChats&&state.chats.length&&!new URLSearchParams(location.search).has('new'))session=structuredClone(state.chats[0]);applySettings();renderChats();renderChat();bind();document.body.dataset.maxgReady='true';
 const support=desktopMode?await engine.check().catch(error=>({supported:false,reason:error.message})):await checkWebGPU();$('compatibilityNotice').hidden=support.supported;$('compatibilityNotice').textContent=support.reason;
 await shellReady;
 backgroundUpdateCheck();
 if(navigator.storage?.persist)navigator.storage.persist().catch(()=>{});
 setInterval(()=>{const retainForLearning=!document.hidden&&(state.settings.hourlyLearning||state.jobs.some(job=>job.enabled));if(idleModel.due({ready:engine.ready,busy:Boolean(active||engine.busy||playing||voiceMode||queued||retainForLearning),mode:state.settings.deviceMode,hints:deviceHints()})){idleModel.touch();const epoch=taskEpoch;engine.unload().then(()=>{if(taskEpoch===epoch&&!engine.ready&&!engine.busy)$('modelStatus').textContent=desktopMode?'CPU model released while idle · installed weights kept':'Local AI released while idle · cached downloads kept';}).catch(showError);}},15000);
