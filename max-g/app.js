...
function syncVoiceControls(){
  ...
  const label=$('voiceModeBtn').querySelector('span');if(label)label.textContent=voiceMode?'End conversation':'Talk with MAX-G';
  $('micBtn').setAttribute('aria-pressed',String(Boolean(voice.recognition)));
}
function pauseVoiceOutput(){audioAttempt++;voiceSession++;clearTimeout(voiceListenTimer);voiceListenTimer=null;voiceStarting=false;voiceMode=false;voice.stop();syncVoiceControls();}
...
function renderMessage(message,{authorized=false}={}){
  ...
  if(research){
    const showCards=()=>{article.classList.add('message-research');const cards=renderResearch(research,{authorized});if(cards)article.append(cards);};
    if(message.showArticles===true)showCards();
    else{const reveal=button('Show articles',()=>{message.showArticles=true;showCards();reveal.remove();record();},'text-button');reveal.dataset.showArticles='';article.append(reveal);}
  }
...
function scrollBottom(){requestAnimationFrame(()=>{const scroll=$('conversationScroll'),latest=$('messages').lastElementChild;if(latest?.dataset.replyComplete==='true'||latest?.classList.contains('message-weather')||latest?.classList.contains('message-research')){const narration=latest.querySelector('.message-content:not([hidden])');const anchor=latest.dataset.replyComplete==='true'&&!latest.classList.contains('message-weather')||latest.classList.contains('message-research')?(narration?.textContent.trim()?narration:latest.querySelector('.research-story-header'))||latest:latest;scroll.scrollTop+=anchor.getBoundingClientRect().top-scroll.getBoundingClientRect().top-8;}else scroll.scrollTop=session.messages.length?scroll.scrollHeight:0;});}
  ...
  try{
    ...
    if(social){answer=social.text;answerModel='Local greeting';setOrb('idle',social.emotion);}
      ...
      if(answer.trim()==='[SEARCH]'&&!searchFailed&&!files.length&&!sources.length&&researchPlan(query,{explicit:true,personal:consultation.personal||smallTalk(text)})){
        ...
        researchData=await publicResearch.lookup(query,{mode:/\bnews\b/i.test(query)?'news':'topic',signal:run.signal,onPartial:showResearch});checkRun(run);sources=researchSources(researchData);
    ...
    const delivery=replyDelivery({question:text,answer,cue:run.deliveryCue||social||consultationOpener||{},joke:writing?.format==='joke',humor:state.settings.humor});setOrb('idle',delivery.emotion);
