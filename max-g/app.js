...
import {researchPlan,researchSources,researchAnswer} from './research-routing.js';
...
function refreshInferenceUI(){
  ...
  $('inferenceDescription').textContent=usingCloud()?'Cloudflare processes your new messages, recent cloud-session replies and public sources. Saved memories, profile details and attachments stay local.':'Local AI processes conversation on this device. Web search and connected tools use the internet when allowed.';
...
function setOrb(status='idle',emotion){orbit.setMood(status,emotion);$('orb').dataset.state=status;if($('orbStatus'))$('orbStatus').textContent={idle:'MAX-G is here',thinking:usingCloud()?'Thinking with Cloudflare AI…':'Thinking locally…',listening:'Listening to you',speaking:'Speaking to you'}[status]||status;if(emotion&&EMOTIONS.includes(emotion))$('orb').dataset.emotion=emotion;}
...
const voice=new LocalVoice({preferCompanionNeural:desktopMode,onState:status=>{activityFrame.voice(status);setOrb(status);if(status==='speaking')audioFeedback('Playing voice…','playing',true);else if(status==='thinking')audioFeedback('Preparing speech on this device…','preparing',true);else if(status==='idle'){if($('stopSpeechBtn'))$('stopSpeechBtn').hidden=true;if($('voiceFeedback')?.dataset.kind==='playing')audioFeedback('Playback finished. If you heard nothing, open Sound help.');}},onProgress:text=>{if(/^Idle voice/i.test(text)&&$('voiceFeedback')?.dataset.kind==='error')return;const node=document.querySelector('.voice-status');if(node)node.textContent=text;const preparing=!/^Idle voice|Natural voice ready/i.test(text);audioFeedback(text,preparing?'preparing':'info',preparing);},onLevel:level=>$('orb').style.setProperty('--speech-level',String(level)),helper:(...args)=>connectorHub.voiceRequest(...args),permission});
...
const updates=createAppUpdates({desktop:desktopMode,request:operation=>connectorHub.updateRequest(operation),
  ...
  beforeApply:async()=>{
    ...
    if(!await persist())throw Error('MAX-G could not save your changes. Keep this window open and export your data before updating.');
  },onChange:value=>{
...
function renderMessage(message,{authorized=false}={}){
  ...
  if(research){article.classList.add('message-research');const cards=renderResearch(research,{authorized});if(cards)article.append(cards);if(message.model==='Public source briefing')content.hidden=true;}
  if(message.sources?.length&&!research){if(card){const details=element('details','','weather-sources');details.append(element('summary','Weather sources'),sourceNodes(message.sources));article.append(details);}else article.append(sourceNodes(message.sources));}
...
function scrollBottom(){requestAnimationFrame(()=>{const scroll=$('conversationScroll'),latest=$('messages').lastElementChild;if(latest?.classList.contains('message-weather')||latest?.classList.contains('message-research')){const narration=latest.querySelector('.message-content:not([hidden])');const anchor=latest.classList.contains('message-research')?(narration?.textContent.trim()?narration:latest.querySelector('.research-story-header'))||latest:latest;scroll.scrollTop+=anchor.getBoundingClientRect().top-scroll.getBoundingClientRect().top-8;}else scroll.scrollTop=session.messages.length?scroll.scrollHeight:0;});}
...
async function generateCloud(query,sources,files,run,contentNode,task,outputLimit){
  ...
  let native=false;
  if(!geminiAccessToken&&desktopMode&&typeof connectorHub.nativeCloudflareRequest==='function'){
    const status=await connectorHub.nativeCloudflareRequest('status',{}, {signal:run.signal});checkRun(run);native=status?.configured===true;
  }
  ...
  const started=performance.now();let firstToken=null,paint=0,last='';
  setOrb('thinking','thoughtful');$('modelStatus').textContent='Cloudflare AI · connecting and preparing reply…';
  const onToken=(_delta,whole)=>{if(active!==run||run.signal.aborted)return;if(firstToken===null)firstToken=performance.now();last=whole;if(!paint)paint=requestAnimationFrame(()=>{paint=0;if(active!==run||run.signal.aborted)return;const text=decodeReply(last);contentNode.textContent=text.trim()==='[SEARCH]'?'':sources.length?cleanWebReply(text):text;scrollBottom();});};
  try{
    ...
    $('modelStatus').textContent=`Cloudflare AI: ${((performance.now()-started)/1000).toFixed(1)} s${firstToken===null?'':` · first text ${((firstToken-started)/1000).toFixed(1)} s`}`;
...
async function checkCloudConnection(){
  ...
  try{
    ...
    const text=!health.ready?'Cloudflare AI needs owner setup. Local AI remains available.':access?'Cloudflare AI is configured. Send a question to test an answer; free limits and availability still apply.':'Cloudflare AI is configured. Add your MAX-G support access token below, or import its .key file for this session.';
...
function renderCloudflareSettings(signal){
  const card=textCard('Cloudflare AI · free allowance','Cloudflare Workers AI runs GLM 4.7 Flash on Cloudflare’s servers. It does not require a model download on this device.');
  card.dataset.cloudflareSettings='';
  card.append(element('p','When Cloudflare AI is selected, Send shares your current message, recent messages from this cloud session, and relevant public source excerpts. Saved profile details, memories, imported old chats and attachments are excluded. Local AI keeps conversation on this device.','muted'));
  card.append(element('p','The free daily allowance and MAX-G’s shared request limit can run out. MAX-G stops with an explanation; it does not upgrade to paid service or silently switch to Gemini or another provider.','small muted'));
  const status=element('p',desktopMode?'The installed Mac can use its protected Cloudflare connection. A browser uses the session token below.':'Use your MAX-G support access token below, or import its .key file. It is forgotten when this tab closes.','muted');status.id='cloudflareSetupStatus';status.setAttribute('role','status');
  card.append(status,button('Check Cloudflare AI setup',()=>checkCloudConnection()));
  return card;
}
function renderGeminiSettings(signal){
  const card=textCard('Cloud AI session access & optional Gemini','The MAX-G support access token authorizes Cloudflare AI and optional Gemini support. It is separate from any provider API key. Ask Gemini still opens a separate review step and sends only the question you approve to Google.');
  ...
  const tokenFile=element('input');tokenFile.type='file';tokenFile.accept='.key,.txt,text/plain';tokenFile.hidden=true;tokenFile.setAttribute('aria-label','Import MAX-G access-token file');
  ...
  tokenFile.addEventListener('change',async()=>{const file=tokenFile.files?.[0];tokenFile.value='';if(!file)return;try{if(file.size>4096)throw Error('Choose the small MAX-G .key file containing only your access token.');const value=await file.text();if(signal.aborted)return;useToken(value);}catch(error){status.textContent=error.message;showError(error);}});
    ...
    if(social){answer=social.text;answerModel='Local greeting';setOrb('idle',social.emotion);}
      ...
      if(plan){
        ...
        if(state.settings.permissions.internet==='deny'&&localFallback){searchFailed=true;}
          ...
          try{researchData=await publicResearch.lookup(publicQuery,{mode:plan.mode,signal:run.signal,onPartial:showResearch});checkRun(run);sources=researchSources(researchData);}
      ...
      if(researchData){
        answer=researchAnswer(researchData);answerModel='Public source briefing';
        ...
        if(usingCloud()||engine.readyFor(state.settings.model)){
          ...
          catch(error){checkRun(run);if(error.name==='AbortError')throw error;rendering.content.textContent='';rendering.content.hidden=true;toast(usingCloud()?'Source articles are ready. '+error.message:'The source briefing is ready. Local AI narration is unavailable for this reply.');if(usingCloud()&&error.code==='AI_UNAUTHORIZED'&&!cloudAuthPrompted){cloudAuthPrompted=true;openAISettings();}}
        }
...
function newConversation(){if(active)return toast('Stop the active operation first.');clearPublicTopic();locationPending=null;locationContext=null;locationHub.retireSelection();stopPlay();session={id:crypto.randomUUID(),title:'New conversation',messages:[]};lastAnswer='';attachments=[];renderAttachments();setView('chat');renderChat();$('messageInput').focus();}
...
function renderSettings(tab=settingsTab){pauseVoiceOutput();stopPlay();locationHub.cancel();settingsController?.abort();settingsController=new AbortController();const settingsSignal=settingsController.signal;settingsTab=tab;const panel=$('settingsContent');panel.replaceChildren();for(const button of document.querySelectorAll('.settings-tab'))button.classList.toggle('active',button.dataset.tab===tab);const pending={};const add=(key,label,options)=>{const f=field(label,state.settings[key],options);panel.append(f.wrap);pending[key]=f.input;return f.input;};
 ...
 if(tab==='connection'){
  ...
  panel.append(toggle('Illustrated public answers · automatic web lookup',state.settings.illustratedAnswers,v=>{state.settings.illustratedAnswers=v;applySettings();persist();}),element('p','Show source articles and available photos for public factual questions, without waiting for AI. Turn this and web-first answers off to prefer the selected model’s existing knowledge. Internet permissions still apply.','small muted'));
  const connection=add('searchConnection','Search connection',{options:[{value:'default',label:'MAX-G Cloudflare (default)'},{value:'companion',label:'Paired Mac companion'}]});
  ...
  panel.append(element('p','MAX-G sends the current public query to the endpoint shown above, without conversation history, notes or attachments. MAX-G’s Cloudflare Worker retrieves public search results over this device’s internet connection. The free service has shared request limits and can be temporarily unavailable. The optional paired companion uses Bing. Internet access still follows Settings → Permissions. News also retrieves BBC News and Sky News feeds through MAX-G’s Cloudflare connection. Topic background and attributed pictures come from Wikipedia and Wikimedia Commons. Only the current public topic is shared. Publisher photos can be archival; missing or ambiguous photos are omitted. Weather and places use their own public services.','muted'),button('Test web search',async()=>{const url=pending.proxyURL.value,connection=pending.searchConnection.value;await permission('internet');const run=begin('search-test');try{const result=await searchPublic('site:docs.python.org Python documentation',run.signal,url,connection);checkRun(run);toast(`Web search connected: ${result.results.length} public results.`);}finally{finish(run);}}));
 }
...
async function factoryReset(){
  ...
  try{
    ...
    clearPublicTopic();state=freshState();if(desktopMode)state.settings=desktopDefaults(state.settings);locationHub.forget();session={id:crypto.randomUUID(),title:'New conversation',messages:[]};attachments=[];workspace=[];folderHandle=null;selectedSkill='';lastAnswer='';evaluationProgress='';
...
boot().catch(error=>{showError(error);$('compatibilityNotice').hidden=false;$('compatibilityNotice').textContent='MAX-G could not initialize: '+error.message;}).finally(()=>activityFrame.set('startup',false));
