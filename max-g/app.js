...
function scrollBottom(){requestAnimationFrame(()=>{const scroll=$('conversationScroll'),latest=$('messages').lastElementChild;if(latest?.classList.contains('message-weather'))scroll.scrollTop+=latest.getBoundingClientRect().top-scroll.getBoundingClientRect().top;else scroll.scrollTop=session.messages.length?scroll.scrollHeight:0;});}
...
async function submit(text=$('messageInput').value,selected=attachments){
  ...
  const weatherIntent=selected.length?null:weatherRequest(text,normalizeLocationSettings(state.settings.locations).autoLocate!==false?'':state.settings.weatherCity);
  ...
  session.messages.push({role:'user',content:text,sources:[]});renderMessage(session.messages.at(-1));$('welcome').classList.add('compact','has-messages');dockOrb();const rendering=renderMessage({role:'assistant',content:'',sources:[]});let answer='',sources=[],answerModel='Built-in tool',publicQuery='',searchFailed=false;
  try{
    ...
    const quick=files.length?null:(basicDeviceAnswer(text)||quickAnswer(text,{units:state.settings.unitSystem})),request=files.length?null:weatherRequest(text,state.settings.weatherCity);
    ...
    if(social){answer=social.text;answerModel='Local greeting';setOrb('idle',social.emotion);}
      ...
      if(searchBeforeReply(text,{onlineFirst:state.settings.onlineFirst,explicit:Boolean(research),personal:smallTalk(text),hasFiles:Boolean(files.length)})){
        const localFallback=allowLocalSearchFallback(text,{explicit:Boolean(research)});
        if(state.settings.permissions.internet==='deny'&&localFallback){searchFailed=true;}
        else{await permission('internet');checkRun(run);publicQuery=research?.[1]||text;$('modelStatus').textContent='Checking public web evidence…';rendering.content.textContent='Checking current sources…';
          try{sources=(await searchPublic(publicQuery,run.signal)).results;checkRun(run);}
      ...
      if(answer.trim()==='[SEARCH]'&&!searchFailed&&!files.length&&!sources.length){await permission('internet');checkRun(run);publicQuery=text;sources=(await searchPublic(text,run.signal)).results;checkRun(run);answer=await generate(text,sources,[],run,rendering.content);}
...
function bind(){
 ...
 window.addEventListener('pagehide',()=>{stop();fastWeather.clear();forgetGeminiToken();locationHub.forget({persist:false});releaseOwnership?.();engine.unload().catch(()=>{});});window.addEventListener('pageshow',event=>{if(event.persisted)location.reload();});
}
...
async function prepareOfflineShell(){
  ...
  try{await Promise.race([(async()=>{
    ...
    registration.addEventListener('updatefound',()=>{const worker=registration.installing;worker?.addEventListener('statechange',()=>{if(worker.state==='installed'&&navigator.serviceWorker.controller)toast('A MAX-G update is ready. Close all MAX-G windows and reopen to apply it.');});});
