...
function renderMessage(message,{authorized=false}={}){
  ...
  if(message.role==='assistant'&&message.content){
    ...
    actions.append(button('Read aloud',()=>{pauseVoiceOutput();return speakMessage(message);},'text-button'),button('Copy',()=>navigator.clipboard.writeText(message.content).then(()=>toast('Copied.')),'text-button'),button('Save note',()=>saveNote(message.content,message.sources),'text-button'));
...
async function submit(text=$('messageInput').value,selected=attachments){
  ...
  try{
    ...
    const delivery=replyDelivery({question:text,answer,cue:run.deliveryCue||social||consultationOpener||{},joke:writing?.format==='joke',humor:state.settings.humor});setOrb($('orb').dataset.state==='speaking'?'speaking':'idle',delivery.emotion);
