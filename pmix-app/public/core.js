(function(root) {
  ...
  const CHANNELS = {ALL:'All channels',CO:'Carry-Out',DI:'Dine-In',DD:'DoorDash',CT:'Catering',HH:'Happy Hour',OO:'Online Order',BAR:'Bar',DT:'Drive Thru',UNMAPPED:'Unassigned dining option',OTHER:'Other',WI:'Walk-In',CI:'Call-In',KSK:'Kiosk',PATIO:'Patio',IN:'Inside restaurant',CTDEL:'Catering Delivery',CTPU:'Catering Pick Up',CTONL:'Catering Online'};
  ...
  function channelCode(v) {const s=clean(v).toUpperCase().replace(/[\s_\/-]/g,''); return ({WI:'WI',WALKIN:'WI',CI:'CI',CALLIN:'CI',ONL:'OO',ONLINE:'OO',KSK:'KSK',KIOSK:'KSK',PATIO:'PATIO',IN:'IN',INSIDERESTAURANT:'IN',CTDEL:'CTDEL',CATERINGDELIVERY:'CTDEL',CTPU:'CTPU',CATERINGPICKUP:'CTPU',CTONL:'CTONL',CATERINGONLINE:'CTONL',CO:'CO',CARRYOUT:'CO',TAKEOUT:'CO',DIN:'DI',DI:'DI',DINEIN:'DI',DININGROOM:'DI',OO:'OO',ONLINEORDER:'OO',BAR:'BAR',DT:'DT',DRIVETHRU:'DT',TOGO:'CO',ONLINEORDERING:'OO',DD:'DD',DOORDASH:'DD',CT:'CT',CATERING:'CT',HH:'HH',HAPPYHOUR:'HH',ALL:'ALL',ALLCHANNELS:'ALL'})[s] || null;}
  ...
  function validateBatch(batch, batches=[],replaceId=null) {
    ...
    for(const b of batches){if(b.id===replaceId)continue;if((batch.meta?.metricOnly||'quantity')!==(b.meta?.metricOnly||'quantity'))continue;const period=batch.start<=b.end&&b.start<=batch.end;if(period&&batch.fileHash&&b.fileHash&&batch.fileHash!==b.fileHash&&(batch.start!==b.start||batch.end!==b.end))continue;const stores=(batch.aggregateStores||batch.stores).some(s=>(b.aggregateStores||b.stores).includes(s));const channel=batch.meta?.toast&&b.meta?.toast?(!(batch.meta.diningDetail&&b.meta.diningDetail)||batch.channel===b.channel)&&batch.meta.sourceMenus.some(m=>b.meta.sourceMenus.includes(m)):batch.channel==='ALL'||b.channel==='ALL'||batch.channel===b.channel;const part=batch.part==='ALL'||b.part==='ALL'||batch.part===b.part;if(period&&stores&&channel&&part)throw Error('This overlaps an existing capture ('+b.label+'). Remove that capture before replacing it, or use a different period/channel/day part.');}
  }
  function locations(batches=[]){return [...STORES,...[...new Map(batches.filter(b=>b.aggregateStores).map(b=>[b.stores[0],[b.stores[0],b.aggregateStores.join(' + ')+' (combined only)','Combined']])).values()]];}
  function report(batches,filter={}){
    ...
    const stores=locations(batches).filter(x=>(!filter.region||filter.region==='All'||x[2]===filter.region)&&(!filter.stores||filter.stores.includes(x[0]))).map(x=>x[0]);
  ...
  const api={STORES,locations,CHANNELS,PARTS,clean,storeCode,channelCode,qty,money,dayPart,csv,matrixFromRows,hierarchyFromRows,normalized,servingType,basket,attachForms,validateBatch,report,formsReport};root.PMIX=api;if(typeof module!=='undefined')module.exports=api;
