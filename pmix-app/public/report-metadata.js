(function(root){
  ...
  function read(name,content=''){
    ...
    const namedParts=[...String(name).replace(/\.(xlsx|csv|tsv|pdf)$/i,'').matchAll(/(?:[ _-])(BK|LN|DN)(?=[ _-]|$)/g)].map(m=>m[1]);
  ...
  function title({metric='quantity',comparison=false,groups=[],special='all',parts=[],stores=[],view='items'}={}){
    let subject=metric==='both'?'PMIX & NET SALES':metric==='netSales'?'NET SALES':special==='bb'?'BB PMIX':groups.length===1?(/breakfast burrito/i.test(groups[0])?'BB PMIX':groups[0].replace(/to go bev/i,'DRINKS').toUpperCase()+' PMIX'):'FULL MENU PMIX';
    if(subject.length>35)subject=metric==='netSales'?'NET SALES':'PMIX';
    const suffix=comparison?'COMPARISON':view==='channels'?'BY DINING OPTION':parts.length===1?parts[0]:stores.length===1?stores[0]:'';
    return subject+(suffix?' · '+suffix:'');
  }
  root.PMIX_METADATA={read,dates,valid,headerText,shortDate,range,title};
